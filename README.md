...
- `board`：唯讀任務看板，依狀態（待辦 / 進行中 / 已完成 / 已取消）分欄，卡片顯示類型 / 優先級 / 標籤 / 負責人等，支援前端搜尋與類型 / 優先級篩選，重新整理即更新。
...
| `board [--port n] [--open]` | 啟動唯讀任務看板（依狀態分欄，重新整理即更新） |
...
taskcli board --open                           # 開唯讀任務看板（依狀態分欄、可搜尋/篩選）
