...
{
  "repoPath": "/Users/carl/Dev/CMG/TaskCli",  // repo 根目錄（含 .taskcli/ 的目錄）
  "allowedUserIds": ["U0XXXXXXX"]              // 允許操作的 Slack user ID（你個人）
}
```

> 註：`repoPath` 指向 repo 根目錄而非 `.taskcli/` 本身，因為既有 command 函式吃的是 root 並內部以 `tasksDir(root)` 等推導路徑；bot 以 `findRoot(repoPath)` 驗證並取得 root。
...
- `repoPath`（repo 根目錄）經 `findRoot` 可定位到 `.taskcli/`。
- `allowedUserIds` 非空陣列。
